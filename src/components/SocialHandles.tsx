...
// Helper to extract username/handle from social media URLs
  ...
  try {
    ...
    switch (platform) {
      case 'linkedin':
        ...
        const linkedinMatch = cleaned.match(/linkedin\.com\/(in|company)\/([^/?]+)/)
...
const SocialHandles = ({
  ...
  const iconSize = 14
  const hasAnySocial = linkedin || twitter || instagram || github

  if (!hasAnySocial) return null

  return (
    <div className="social-handles">
      <div className="social-handles-title" style={{ color: textColor }}>
        Connect
      </div>
      <div className="social-handles-list">
        ...
        {github && (
          <div className="social-handle-item">
            ...
            <span style={{ color: textColor }}>{extractHandle(github, 'github')}</span>
          </div>
        )}
      </div>
    </div>
